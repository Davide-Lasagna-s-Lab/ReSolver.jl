@testset "Optimisation trace        " begin
    ...
    push!(t1, [π], 0, 0, 0.5, 1, 1, 0, 0)
    push!(t2, [π], 0, 0, 0.5, 1, 1, 5, 15)
    ...
    @test t1[1] == (0, 0, 0.5, 1, 1, nothing)
    @test t2[1] == (5, 15, 0.5, 1, 1, π^2)
    push!(t1, [2π], 1, 0.1, 0.5, 1, 1, 0, 0)
    push!(t2, [2π], 1, 0.1, 0.5, 1, 1, 5, 15)
    @test length(t1) == length(t2) == 2
    @test t1[2] == (1, 0.1, 0.5, 1, 1, nothing)
    @test t2[2] == (6, 15.1, 0.5, 1, 1, 4*π^2)
