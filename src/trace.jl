...
# ~~~ State printing ~~~
function print_state(io, state::Tuple{Int, Float64, Float64, Float64, Float64, T}) where {T}
    @printf io "|%10d   |  %5.5e  |   %5.2e  |  %5.5e  |  %5.5e  |\n" state[1] state[2] state[3] state[4] state[4]
    flush(io)
end
